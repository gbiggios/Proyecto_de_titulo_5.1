...
class BitacoraForm(FlaskForm):
    comentario_bitacora = TextAreaField('Comentario de la Bitácora', validators=[DataRequired()])
    submit = SubmitField('Guardar Bitácora')
    
class ClaseForm(FlaskForm):
    fecha = DateField('Fecha', format='%Y-%m-%d', validators=[DataRequired()])
    comentario_bitacora = TextAreaField('Comentario de Bitácora')
    suspendida = HiddenField('Suspendida', default='0')  # Campo oculto para la suspensión
